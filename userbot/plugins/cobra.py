...
# 🇨 🇭 🇦 🇱 🇴      🇸 🇮 🇷    🇵 🇱 🇪 🇦 🇸 🇪    🇬 🇪 🇹 🇴 🇺 🇹    
    ...
    async def opner(event):
            if event.query.user_id == bot.uid :
                ...
                await event.edit("`>>>\n\nReopened The Main Menu of \n©WHITEDEVIL` ", buttons=dc)
            else:
                reply_pop_up_alert = "Please get your own Userbot,for more info visit @WHITEDEVILUSERBOT!"
  ...
  #       🇮 🇹 🇳 🇦    🇰 🇾 🇺   🇸 🇵 🇾     🇰 🇷    🇷 🇭 🇪     🇭 🇴      🇸 🇭 🇦 🇦 🇩 🇮    🇰 🇷 🇳 🇮    🇭    🇰 🇾 🇦   🇧 🇸 🇩 🇰 
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("Userbot"):
            ...
            result = builder.article("© White Devil Userbot Help",text="{}\nCurrently Loaded Plugins: {}".format(query, len(CMD_LIST)),buttons=dc,link_preview=False)
            await event.answer([result] if result else None)
        else:
              reply_pop_up_alert = "Terii maa ka bhosra sala apna userbot bnawo lwde...,for more info visit @Whitedeviluserbot! 😎😎"
              await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            Devil = "Sur jii apna userbot bnawo, pehli fursaf me nikkal    for more info visit @WHITEDEVILUSERBOT!"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            await event.edit(buttons=dc)
              ...
              Thewhite = "Please get your own Userbot😁😁,for more info visit @WHITEDEVILUSERBOT! 😎😎"
 ...
 #hehehehehhehhehhehe   
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            atul= "Please get your own Userbot😁😁,for more info visit @WHITEDEVILUSERBOT! 😎😎"
        ...
        try:
            if event.query.user_id == bot.uid :
                ...
                reply_pop_up_alert = "Please get your own Userbot, and don't use mine for more info visit @WHITEDEVILUSERBOT!"
...
#  🇦 🇷 🇪      🇧 🇸 🇩 🇰      🇮 🇸 🇸 🇪    🇰 🇦 🇳 🇬  🇲 🇦 🇹   🇰 🇷    🇷 🇪   🇲 🇨 
                    ...
                    try:
                        ...
                        await event.edit("Error{}".format(shortname, str(e))+ "WhiteDevil Has Successfully loaded" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
    ...
    async def on_plug_in_callback_query_handler(event):
              if event.query.user_id == bot.uid :
                    ...
                    try:
                        ...
                        await event.edit("Error{}".format(shortname, str(e)) +"WhiteDevil Has Successfully unloaded"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  𝕭𝖆𝖈𝖐 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 𝕷𝖔𝖆𝖉 ",data="load({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @WHITEDEVILUSERBOT!"
